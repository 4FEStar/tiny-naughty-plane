(function () {
	function Asteroids() {
		...
		*/
		...
		var lastUpdate = new Date().getTime(); // 用于标识上次更新时间
		var forceChange = false; // 标识是否需要更新画布

		// 关键在update方法，每次刷新整个canvas
		this.update = function () {
			// ==
			// logic
			// ==
			var nowTime = new Date().getTime();
			var tDelta = (nowTime - lastUpdate) / 1000; // 时间差,单位为秒
			lastUpdate = nowTime;
